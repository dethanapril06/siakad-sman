...
<head>
    ...
    <title>Laporan Keterlambatan - {{ $selectedMengajar->mataPelajaran?->nama }} - {{ $selectedMengajar->kelasAkademik?->nama_lengkap }}</title>
...
<body>
    @php
        $backRoute = route('guru.laporan-keterlambatan.index', ['mengajar_id' => $selectedMengajar->id]);
    ...
    <div class="report-page">
        ...
        <div class="report-title">
            <h3>REKAPITULASI CATATAN KETERLAMBATAN SISWA</h3>
        </div>
        ...
        <table class="meta-table">
            ...
            <tr>
                <td><strong>Total Kejadian</strong></td>
                <td>:</td>
                <td colspan="4">{{ $keterlambatans->count() }} Kali Keterlambatan Tercatat</td>
            </tr>
        </table>
        ...
        <table class="data-table">
            <thead>
                <tr>
                    ...
                    <th>Nama Siswa</th>
                    <th>Keterangan / Alasan Keterlambatan</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($keterlambatans as $index => $item)
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td>{{ \Carbon\Carbon::parse($item->pertemuan?->tanggal)->isoFormat('DD/MM/YYYY') }}</td>
                        <td>Pertemuan ke-{{ $item->pertemuan?->pertemuan_ke }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="6" style="padding: 20px;">Tidak ada data catatan keterlambatan.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
