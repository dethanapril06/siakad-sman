...
<head>
    ...
    <title>Laporan Absensi - {{ $selectedMengajar->mataPelajaran?->nama }} - {{ $selectedMengajar->kelasAkademik?->nama_lengkap }}</title>
...
<body>
    @php
        $backRoute = route('guru.laporan-absensi.index', ['mengajar_id' => $selectedMengajar->id]);
        $sekolah = \App\Models\Sekolah::first();
    @endphp

    <div class="no-print-bar">
        <div>
            <strong>Laporan Rekapitulasi Presensi / Absensi</strong>
    ...
    <div class="report-page">
        ...
        <table class="meta-table">
            ...
            <tr>
                ...
                <td>:</td>
                <td colspan="4">{{ $totalPertemuan }} Pertemuan terlaksana</td>
            </tr>
        </table>
        ...
        <table class="data-table">
            <thead>
                <tr>
                    ...
                    <th style="width: 80px;">Total Hadir</th>
                    <th style="width: 90px;">% Kehadiran</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($rekapSiswa as $item)
                    <tr>
                        <td>{{ $item['no'] }}</td>
                        ...
                        <td>{{ $item['izin'] }}</td>
                        <td>{{ $item['alpa'] }}</td>
                        <td class="fw-bold">{{ $item['total_hadir'] }}</td>
                        <td class="fw-bold">{{ $item['persentase'] }}%</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="10" style="padding: 20px;">Data absensi belum tersedia.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
