...
<head>
    ...
    <title>Laporan Jadwal Mengajar Guru - {{ $semester?->tahunAkademik?->nama }}</title>
...
@endphp
    ...
    <div class="no-print-bar">
        <div>
            <strong>Laporan Jadwal Mengajar Guru</strong>
    ...
    <div class="report-page">
        ...
        <div class="report-title">
            <h3>LAPORAN JADWAL MENGAJAR GURU</h3>
            <p style="margin: 3px 0; font-size: 9.5pt;">Tahun Pelajaran: {{ $semester?->tahunAkademik?->nama }} - Semester {{ ucfirst($semester?->nama ?? '') }}</p>
        </div>

        @forelse ($jadwalPerGuru as $group)
            <div style="margin-bottom: 22px;">
                <div style="font-size: 10pt; font-weight: 700; margin-bottom: 5px; display: flex; justify-content: space-between;">
                    <span>Guru: {{ $group['guru']->nama }} (NIP. {{ $group['guru']->nip ?? '-' }})</span>
                    <span>Total Beban: {{ $group['total_jam_per_minggu'] }} Jam/Minggu ({{ $group['jumlah_jadwal'] }} Sesi)</span>
                </div>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        @forelse ($group['jadwals'] as $idx => $j)
                            <tr>
                                <td>{{ $idx + 1 }}</td>
                                <td class="fw-bold">{{ ucfirst($j->hari) }}</td>
                                <td>{{ substr($j->jam_mulai, 0, 5) }} - {{ substr($j->jam_selesai, 0, 5) }}</td>
                                <td class="text-start">{{ $j->mengajar?->mataPelajaran?->nama ?? '-' }}</td>
                                <td>{{ $j->mengajar?->kelasAkademik?->nama_lengkap ?? '-' }}</td>
                                <td>{{ $j->ruangan?->nama ?? '-' }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="text-center py-2">Belum ada jadwal untuk guru ini.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        @empty
            <div style="text-align: center; padding: 30px;">Tidak ada data jadwal guru yang sesuai filter.</div>
        @endforelse
